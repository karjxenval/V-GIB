    ...
    for seed in seeds:
        for noise in noise_levels:
            for beta in betas:
                for gamma in gammas:
                    for z_dim in z_dims:
                        print(f"\n▶ Running seed={seed} noise={noise} β={beta} $\gamma$={gamma} z={z_dim}")
...
print(f" Mean η_eff = {mean_eta:.3f}  (values  $>1 \Rightarrow $ V-GIB more sample-efficient)")
...
# Sanity-Check Baseline vs V-GIB
sns.barplot(
    x=["Random", "Best V-GIB"],
...
# Information–Geometry Trade-off: Accuracy vs KL (bubble = Curvature)
sns.scatterplot(
    data=total_df,
    x="kl",
    y="acc",
...
# ---------------------------------------------------------------------
# Fashion-MNIST (proxy for natural manifold data)
# ---------------------------------------------------------------------
...
# %%
# ---------------------------------------------------------------------
# CIFAR-10 with Concept Alignment
# ---------------------------------------------------------------------
...
# --- Data loading (no re-download if already cached) ---
transform = transforms.Compose([
